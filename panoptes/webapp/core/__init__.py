...
class BaseField(object):
    ...
    def is_required(self):
        ...
        if self.required:
            if self._value is not None and self.default is not None:
...
class BaseDocument(object):
    ...
    def save(self):
        ...
        if insertion_data and not self._id.value:
            ...
            self.db[self.collection].update_one({"_id":self._id.value},{"$set":insertion_data})
        else:
    ...
    def populate_from_form(self,form):
        ...
        for item in self._get_obj_attrs():
            if item.dbfield != "_id" and item.validates():
                insertion_data.update(item.to_dict())
